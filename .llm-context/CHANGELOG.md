...
## Outstanding Work (Phases 3-8)

### Phase 3: Script Creation Guides (9 files, 12-16 hours estimated)

**Status**: Not Started
**Priority**: HIGH

**Tasks:**
- [ ] Create `script-creation/overview.md`
- [ ] Create `script-creation/research-methodology.md`
- [ ] Create `script-creation/dependency-analysis.md`
- [ ] Create `script-creation/resource-planning.md`
- [ ] Create `script-creation/os-selection.md`
- [ ] Create `script-creation/installation-patterns.md`
- [ ] Create `script-creation/template-guide.md`
- [ ] Create `script-creation/testing-validation.md`
- [ ] Create `script-creation/best-practices.md`

**Purpose**: Enable LLMs to create new service scripts with:
- Service research methodology
- Dependency identification
- Resource planning guidelines
- OS selection criteria
- Installation patterns (generic, no specific app examples)
- Template usage
- Testing procedures
- Best practices
...
**Tasks:**
- [ ] Populate all 26 category stub files created in Phase 1:
  - Add script listings for each category
  - Document common patterns
  - Add resource requirements
  - Add special considerations
  - Add related category references

**Purpose**: Provide category-specific guidance for finding scripts

### Phase 5: Script-Specific Context Files (408 files in 41 sections, 68-72 hours estimated)

**Status**: Not Started
**Priority**: HIGH

**Tasks:**
- [ ] Section-01: Create context for scripts 1-10 (2fauth, actualbudget, adguard, adventurelog, agentdvr, alpine-adguard, alpine-bitmagnet, alpine-caddy, alpine-docker, alpine-forgejo)
- [ ] Section-02: Create context for scripts 11-20 (alpine-garage through alpine-gitea)
- [ ] Section-03: Create context for scripts 21-30 (alpine-grafana through alpine-teamspeak-server)
- [ ] Section-04: Create context for scripts 31-40 (alpine-tinyauth through apache-tomcat)
- [ ] Section-05: Create context for scripts 41-50 (apt-cacher-ng through bazarr)
- [ ] Section-06: Create context for scripts 51-60 (bentopdf through caddy)
- [ ] Section-07: Create context for scripts 61-70 (changedetection through cockpit)
- [ ] Section-08: Create context for scripts 71-80 (coder-code-server through cronicle)
- [ ] Section-09: Create context for scripts 81-90 (cross-seed through domain-monitor)
- [ ] Section-10: Create context for scripts 91-100 (devuan-vm through dotnetaspwebapi)
- [ ] Sections 11-40: Continue through remaining 308 scripts

**Purpose**: Create individual context files for all 408 CT scripts with:
- Basic info (name, slug, categories, description)
- Resources by install method (CPU/RAM/Disk/OS)
- Access info (ports, URLs, credentials)
- Special requirements (GPU, FUSE, TUN, nesting)
- Interactive prompts (all read -p prompts)
- Non-interactive execution examples
- Related scripts (Alpine variants, similar services)
- Update procedure

### Phase 6: Automation Scripts (3 scripts, 4-6 hours estimated)
...
**Tasks:**
- [ ] Create `automation/generate-context.sh`
  - Auto-generate context for new scripts
  - Analyze CT script header
  - Analyze install script for prompts
  - Analyze JSON metadata
  - Generate context file following template
- [ ] Create `automation/update-index.sh`
  - Scan all context files in scripts/
  - Update index.md with new scripts
  - Update category files with new scripts
  - Regenerate special requirement indexes
- [ ] Create `automation/scan-new-scripts.sh`
  - Compare ct/ directory listing with existing context files
  - Identify scripts without context
  - List scripts added by other users
  - **Reminders to run generate-context.sh** for unmapped scripts

**Purpose**: Maintain context system as repository evolves
...
**Tasks:**
- [ ] Update `AGENTS.md` with LLM Context System section (✅ Already completed in Phase 1)
- [ ] Update `CLAUDE.md` (if exists) with LLM context reference
